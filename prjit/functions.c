...
int isIdExist(struct Student sv[], int length, const char* studentId){
    ...
    int isValidEmail(char* email){
        ...
        return countAtGmail <= 1; // Email hop le neu khong co nhieu hon 1 lan @gmail.com
    }
    ...
    for (i = *length; i < *length + newStudents; i++){
        ...
        // Nhap ten sinh vien va kiem tra  
        do {
...
void searchStudent(int length, struct Student sv[]){
    ...
    int i;
    for (i = 0; i < length; i++){
        // Kiem tra nguoi nhap co ton tai hay khong
        if (strstr(sv[i].fullName, namePart) != NULL) {
            // Tim thay thi se hien thi thong tin sinh vien 
            if (!found) {
...
void sortStudentsByName(int length, struct Student sv[]){	
	...
	do{
	    ...
	    scanf("%d", &choicesortStudentsByName);
		switch(choicesortStudentsByName){
			struct Student temp;
			int i;
			int j;
			case 1:
	...
	}while(choicesortStudentsByName!=0);
}
...
void sortTeachersByName(int length, struct Teacher tv[]){
	...
	do{
	    ...
	    printf("\t=====================\n");
		printf("\tEnter your choose: ");
	    scanf("%d", &choicesortTeachersByName);
		switch(choicesortTeachersByName){
			struct Teacher temp;
			int i;
			int j;
			case 1:
